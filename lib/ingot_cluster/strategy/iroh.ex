defmodule IngotCluster.Strategy.Iroh do
  @moduledoc """
  libcluster strategy: membership over **Iroh** (iron / P2P QUIC).
      ...
      config :libcluster,
        topologies: [
          ingot_iroh: [
            strategy: IngotCluster.Strategy.Iroh,
            config: [
              interval: 5_000,
              alpns: ["ingot_cluster/1"]
            ]
          ]
        ]

  Starts `IngotCluster.Iroh` if it is not already running. Node names are
  ...
  defp tick(state) do
    ...
    _ = IngotCluster.hash64(Atom.to_string(self_node))
    ...
    :telemetry.execute(
      [:ingot_cluster, :strategy, :iroh, :tick],
  ...
  defp maybe_start_iroh(cfg) do
    if Process.whereis(IngotCluster.Iroh) do
      ...
      opts = [alpns: Keyword.get(cfg, :alpns, ["ingot_cluster/1"])]

      case IngotCluster.Iroh.start_link(opts) do
        {:ok, _} -> :ok
        {:error, {:already_started, _}} -> :ok
        other -> Logger.debug("IngotCluster.Strategy.Iroh iroh start #{inspect(other)}")
      end
    end
  end
