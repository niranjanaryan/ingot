defmodule IngotCluster.Zenoh do
  @moduledoc """
  ...
  def init(opts) do
    ...
    key = Keyword.get(opts, :key, "ingot_cluster/cluster/**")
    ...
    if available?() and live do
      case open_session(connect, mode) do
        {:ok, session} ->
          ...
          Logger.info("IngotCluster.Zenoh client → #{connect} key=#{key}")
      ...
      Logger.warning("IngotCluster.Zenoh stub (zenohex=#{available?()} live=#{live} connect=#{connect})")
