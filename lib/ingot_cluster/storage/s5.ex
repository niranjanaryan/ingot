defmodule IngotCluster.Storage.S5 do
  @moduledoc """
  S5 blob client. Content-addressed with BLAKE3; HTTP to an S5 node.

      IngotCluster.Storage.S5.put(data, endpoint: "http://127.0.0.1:5050")
      IngotCluster.Storage.S5.get(cid, endpoint: "http://127.0.0.1:5050")
  """

  alias IngotCluster.Storage.CID

  def cid(data) when is_binary(data) do
    %CID{hash: IngotCluster.blake3(data), size: byte_size(data), algo: :blake3}
  end
  ...
  def get(%CID{} = cid, opts) do
    ...
    case :httpc.request(:get, {String.to_charlist(url), []}, http_opts(opts), []) do
      {:ok, {{_, 200, _}, _, body}} ->
        ...
        IngotCluster.Storage.verify(bin, cid)
