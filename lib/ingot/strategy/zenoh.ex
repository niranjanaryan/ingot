defmodule Ingot.Strategy.Zenoh do
  ...
  def init(opts) do
    ...
    key = Keyword.get(cfg, :key, "ingot/cluster/nodes")
  ...
  defp maybe_start_zenoh(cfg) do
    if Process.whereis(Ingot.Zenoh) do
      :ok
    else
      opts = [
        connect: Keyword.get(cfg, :connect, "tcp/127.0.0.1:7447"),
        key: Keyword.get(cfg, :key, "ingot/cluster/**"),
        live: Keyword.get(cfg, :live, false)
      ]

      case Ingot.Zenoh.start_link(opts) do
        {:ok, _} -> :ok
        {:error, {:already_started, _}} -> :ok
        other -> Logger.debug("Ingot.Strategy.Zenoh start #{inspect(other)}")
      end
    end
  end
