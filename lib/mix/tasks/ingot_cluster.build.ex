defmodule Mix.Tasks.IngotCluster.Build do
  @moduledoc false
  use Mix.Task
  @shortdoc "Builds the IngotCluster Zig NIF"
  @recursive true
  ...
  def run(_args) do
    ...
    so = Path.join(app_path, "priv/ingot_cluster_nif.so")
    src = "native/zig/ingot_cluster_nif.zig"
    ok = match?({:ok, %{size: s}} when s > 1024, File.stat(so))
    need = not ok or (File.exists?(src) and newer?(src, so))

    if need do
      Mix.shell().info("Compiling IngotCluster Zig NIF...")
      ...
      IO.write(out)
      if e != 0, do: raise("IngotCluster NIF compile failed")
    end
  end
