defmodule Mix.Tasks.Ingot.Bench do
  ...
  def run(_args) do
    ...
    {km_us, _} = :timer.tc(fn -> for _ <- 1..iters, do: Ingot.key_match(pat, key) end)
    km = iters * 1_000_000 / km_us

    n = 20_000
    {b3_us, _} = :timer.tc(fn -> for _ <- 1..n, do: Ingot.blake3(blob) end)
    {x_us, _} = :timer.tc(fn -> for _ <- 1..n, do: Ingot.xxh3(blob) end)
    {h_us, _} = :timer.tc(fn -> for _ <- 1..n, do: Ingot.hash64(blob) end)
